...
const useAuthProvider = () => {
	...
	const login = (tokenData: TokenData) => {
		...
		api.user
			...
			.then(({ data }) => {
				if (!IS_PRODUCTION)
					console.log("Successfully logged in user: ", data.user.id);
	...
	const loginWithOAuth = () =>
		new Promise<void>((resolve, reject) => {
			if (!import.meta.env.PUBLIC_YNAB_CLIENT_ID)
				return reject("No Client ID found!");
			...
			browser.identity.launchWebAuthFlow(
				...
				(response) => {
					try {
						...
						initialTokenUrl.search = new URLSearchParams({
							code: oauthCode,
						...
						fetch(initialTokenUrl, {
							...
							.then((res) => {
								if (!res.ok)
									throw new Error(
										`Error getting initial token! Status: ${res.status}`,
									);
								return res.json();
							})
							.then((newTokenData) => {
